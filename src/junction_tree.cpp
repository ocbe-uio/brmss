/* Code written by Marco Banterle */
...
std::pair<bool,double> JunctionTree::propose_single_edge_update( arma::uvec& update_idx )
{
    ...
    if( R::runif(0.,1.) < 0.5 )
    {
        ...
        if( numComponents > 1 )
        {
            ...
            randomSep = BVS_subfunc::randIntUniform(1,numComponents-1);
            ...
            x = setCxlS[ BVS_subfunc::randIntUniform(0,setCxlS.size()-1) ];
            y = setCylS[ BVS_subfunc::randIntUniform(0,setCylS.size()-1) ];
    ...
    else          // propose deletion
    {
        ...
        randomComp = BVS_subfunc::randIntUniform(0,numComponents-1);
        ...
        if( setC.size() > 1 )
        {
            ...
            randomIndexes = BVS_subfunc::randWeightedIndexSampleWithoutReplacement(setC.size(),2);
            ...
            {
                Cx = possibleComponents[ BVS_subfunc::randIntUniform(0,possibleComponents.size()-1) ];
                definedCx = true;
            }
            ...
            if( possibleComponents.size() > 0 )
            {
                Cy = possibleComponents[ BVS_subfunc::randIntUniform(0,possibleComponents.size()-1) ];
                definedCy = true;
            }
...
std::pair<bool,double> JunctionTree::propose_multiple_edge_update( )
{
    ...
    if( R::runif(0.,1.) < 0.5 )
    {
        ...
        if( numComponents > 1 )
        {
            ...
            // now choose x and y from Cx\S and Cy\S
            dimX = BVS_subfunc::randIntUniform(1,setCxlS.size());
            dimY = BVS_subfunc::randIntUniform(1,setCylS.size());

            X = BVS_subfunc::randSampleWithoutReplacement( setCxlS.size(), setCxlS, dimX );
            Y = BVS_subfunc::randSampleWithoutReplacement( setCylS.size(), setCylS, dimY );
    ...
    else          // propose deletion
    {
        ...
        {
            // partition C into 3 sets, x y and S (S might be empty)"
            dimY = BVS_subfunc::randIntUniform(2,setC.size());
            dimX = BVS_subfunc::randIntUniform(1,dimY-1);
            dimY -= dimX;
            ...
            std::copy(setC.begin(), setC.end(),
                      std::back_inserter(setS));
            // Select X
            X = BVS_subfunc::randSampleWithoutReplacement( setS.size(), setS, dimX );
            ...
            // Select Y
            Y = BVS_subfunc::randSampleWithoutReplacement( setS.size(), setS, dimY );
            ...
            {
                Cx = possibleCxComponents[ BVS_subfunc::randIntUniform(0,possibleCxComponents.size()-1) ];
                definedCx = true;
            }

            if( possibleCyComponents.size() > 0 )
            {
                Cy = possibleCyComponents[ BVS_subfunc::randIntUniform(0,possibleCyComponents.size()-1) ];
                definedCy = true;
            }
...
void JunctionTree::reRoot()
{
    ...
    newPCS.insert( newPCS.end(), perfectCliqueSequence[ BVS_subfunc::randIntUniform( 1, perfectCliqueSequence.size() -1 ) ] );
...
void JunctionTree::randomJTPermutation()
{
    ...
    if( numComponents > 2 ) // otherwise is at best the reverse of thea bove reRoot move
    {
        ...
        std::shared_ptr<JTComponent> thisComponent = perfectCliqueSequence[ BVS_subfunc::randIntUniform( 1, numComponents -1 ) ];
        ...
        {
            idx = BVS_subfunc::randIntUniform(0,possibleNewParent.size()-1);
